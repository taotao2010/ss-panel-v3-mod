    ...
    public static function GetContent($request, $response, $args)
    {
        ...
        switch ($Elink->type) {
            case -1:
                ...
                $newResponse = $response->withHeader('Content-type', ' application/octet-stream; charset=utf-8')->withHeader('Cache-Control', 'no-store, no-cache, must-revalidate')->withHeader('Content-Disposition', ' attachment; filename=config.conf');//->getBody()->write($builder->output());
